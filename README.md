...
[![Go module](https://raw.githubusercontent.com/wickra-lib/.github/main/profile/badges/go.svg)](https://pkg.go.dev/github.com/wickra-lib/wickra-darwin-go)
...
**Go bindings for the Wickra evolutionary strategy search over its C ABI hub via cgo. A `Darwin` is built from a spec JSON and driven over a JSON boundary, so the result is byte-identical to every other Wickra Darwin binding.**
...
directory of [wickra-darwin](https://github.com/wickra-lib/wickra-darwin) by the release
pipeline: it mirrors the Go sources, the vendored C ABI header (`include/wickra_darwin.h`)
...
Wickra Darwin is research and analytics software. Its outputs are
