/* wickra-darwin C ABI — generated by cbindgen; do not edit by hand. */

#ifndef WICKRA_DARWIN_H
#define WICKRA_DARWIN_H
...
#define WICKRA_DARWIN_ERR_NULL -1

// `cmd_json` was not valid UTF-8.
#define WICKRA_DARWIN_ERR_UTF8 -2

// A panic was caught at the FFI boundary.
#define WICKRA_DARWIN_ERR_PANIC -3

// An opaque handle to a search instance. Created by [`wickra_darwin_new`] and
// destroyed by [`wickra_darwin_free`]; never dereferenced by the caller.
//
...
// with [`wickra_darwin_free`].
//
// # Safety
// `spec_json` must be null or a valid NUL-terminated C string.
WickraDarwin *wickra_darwin_new(const char *spec_json);

// Destroy a search handle. Null is a no-op.
//
// # Safety
// `handle` must be null or a handle previously returned by
// [`wickra_darwin_new`] and not already freed.
void wickra_darwin_free(WickraDarwin *handle);
...
int32_t wickra_darwin_command(WickraDarwin *handle, const char *cmd_json, char *out, uintptr_t cap);

// The library version as a static NUL-terminated string (do not free).
const char *wickra_darwin_version(void);

#ifdef __cplusplus
}  // extern "C"
#endif  // __cplusplus

#endif  /* WICKRA_DARWIN_H */
